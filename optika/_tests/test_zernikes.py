...
class TestClosedForms:
    ...
    def test_spherical(self, position: na.AbstractCartesian2dVectorArray):
        result = optika.zernikes.zernike(11, position)
        rho2 = np.square(position.length)
...
def test_orthonormality():
    ...
    js = range(1, 12)
    basis = {j: optika.zernikes.zernike(j, position) for j in js}
...
def test_zernike_gradient(j: int, position: na.AbstractCartesian2dVectorArray):
    result = optika.zernikes.zernike_gradient(j, position)
    ...
    derivative_x = optika.zernikes.zernike(j, position + dx)
    derivative_x = derivative_x - optika.zernikes.zernike(j, position - dx)
    derivative_x = derivative_x / (2 * h)

    derivative_y = optika.zernikes.zernike(j, position + dy)
    derivative_y = derivative_y - optika.zernikes.zernike(j, position - dy)
